...
export function build({ THREE, root }) {
  ...
  return {
    ...
    update(time) {
      ...
      frozen = focus.includes("attacker") || focus.includes("attack") ||
               focus.includes("plc");
      attackHot = frozen;

      // rudder angle: sweep like the real 0..80 order, or freeze under attack
      ...
      // packets; attack flow only lit when attack is in focus
      for (let i = 0; i < flows.length; i++) {
        const f = flows[i];
        const isAttack = i >= attackFlow;
        f.dot.visible = !isAttack || attackHot;
